...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _ensure_worker(self):
        ...
        # GUI → Worker signals
        self._runner.previewRequested.connect(self._runner.doPreview)
        self._runner.runRequested.connect(self._runner.doRun)
    ...
    def _on_add_clicked(self) -> None:
        # Create a dialog that allows selecting both files and folders
        dlg = QtWidgets.QFileDialog(
            self, "Select album folders or audio files")
        
        # Use Directory mode but show files too
        dlg.setFileMode(QtWidgets.QFileDialog.Directory)
        dlg.setOption(QtWidgets.QFileDialog.ShowDirsOnly, False)
        dlg.setOption(QtWidgets.QFileDialog.DontUseNativeDialog, False)
        
        if dlg.exec():
            selected_paths = dlg.selectedFiles()
            # Filter to only include directories and audio files
            filtered_paths = []
            for path in selected_paths:
                p = Path(path)
                if p.is_dir() or p.suffix.lower() in AUDIO_EXTS:
                    filtered_paths.append(path)
            self._add_to_queue(self._normalize_paths(filtered_paths))
    ...
    def _on_preview_clicked(self) -> None:
        ...
        self._runner.previewRequested.emit(paths, opts)
    ...
    def _on_start_clicked(self) -> None:
        ...
        opts = self.collect_options()
        # Emit signal to worker thread
        self._runner.runRequested.emit(paths, opts)
