...
from app.artwork import detect_art_source, extract_art_to_file, normalize_art_to_jpeg_500
...
class Runner(QtCore.QObject):
    ...
    # Signals for receiving work from GUI thread
    previewRequested = QtCore.Signal(list, dict)  # (path_list, options)
    runRequested = QtCore.Signal(list, dict)      # (path_list, options)
    ...
    def _format_plan_table(self, rows, settings) -> str:
        head = [
            ...
            "Plan Preview",
            "-" * 130,
            f"{'Source':60}  {'Codec':6}  {'Action':22}  {'Art Source':28}  {'JPEG500?':8}  {'Target'}",
            "-" * 130,
        ]
        body = []
        for (plan, art, _src_img, jpeg500) in rows:
            ...
            body.append(
                f"{str(plan.src):60.60}  {plan.src_codec:6}  {action:22}  {art_label:28.28}  {jpeg_ok:8}  {plan.target.name}")
        tail = [
            "-" * 130,
            ...
            "Artwork: 500x500 JPEG (single image)",
        ]
        return "\n".join(head + body + tail)
    ...
    def _process_one(self, plan, art, src_image, jpeg500, settings):
        """
        ...
        try:
            ...
            write_id3_v23_with_apic(
                plan.temp_path, jpeg500, src_tags, settings.strip_frames)
    ...
    def doPreview(self, path_list, options):
    ...
    @QtCore.Slot(list, dict)
    def doRun(self, path_list, options):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=jobs) as exe:
                futs = [exe.submit(self._process_one, plan, art, src_image, jpeg500, settings)
                        for (plan, art, src_image, jpeg500) in rows]

                for fut, (plan, _art, _src_image, _jpeg500) in zip(as_completed(futs), rows):
